...
const DEFAULT_WORDS = ["HELLO", "KISAN KA DUKAN", "LIVE", "COMING", "SOON"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const newColor = {
      r: Math.random() * 155 + 100, // Brighter colors for green theme
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        if (particleIndex < particles.length) {
          ...
          particle.maxSpeed = (Math.random() * 6 + 4) * 1.25
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full relative">
      <canvas
        ref={canvasRef}
        className="w-full max-w-4xl border border-white/10 rounded-2xl shadow-2xl bg-black/80 backdrop-blur-sm"
